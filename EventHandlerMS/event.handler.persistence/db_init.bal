...
public endpoint sql:Client dbEP { database: sql:DB.H2_FILE, host: h2DbLocation,name: h2Database };

// Done as a workaround to init the Database                                                                                                                                        //
int c = initializeDB();

function initializeDB () returns (int) {
    endpoint sql:Client dbEP2 { database: sql:DB.H2_FILE, host: h2DbLocation,name: h2Database};
    ...
    var ret = dbEP2 -> update(query2, null);
    match ret {
        error err => {
            throw err;
        }
